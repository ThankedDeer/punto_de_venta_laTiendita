...
function cargarCategorias() {
  axios.get("http://localhost:3000/api/categorias").then(function(response) {
      const categorias = response.data;
      const darCategoria = document.getElementById("darCategoria");
      const opcionVacia = document.createElement("option");
      opcionVacia.value = "";
      opcionVacia.text = "Selecciona una categoría";
      ...
      darCategoria.addEventListener("change", function() {
        ...
        axios.get(`http://localhost:3000/api/categoria/${idCategoria}`).then(function(response) {
            ...
            actDescripcionCategoria.value = categoria.Descripcion_Categoria;}).catch(function(error) {
            console.log(error);
            Swal.fire({
              title: "Error",
